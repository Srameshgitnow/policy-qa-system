...
questionsRouter.post('/', async (req: Request<{}, {}, QuestionRequest>, res: Response) => {
  try {
    ...
    if (retrievedChunks.length === 0) {
      ...
      return res.json({
        ...
        confidence: 0,
        sources: [],
        relatedPolicies: relatedPolicies.map((p: any) => p.title)
      });
    }
    ...
    const insertResult = await query(
      `INSERT INTO questions (query, answer, confidence, source_ids)
       VALUES ($1, $2, $3, $4)
       RETURNING id, created_at`,
      [userQuery, generatedAnswer.answer, generatedAnswer.confidence, sourceIds]
    );
...
questionsRouter.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      `SELECT id, query, confidence, created_at FROM questions 
