...
async function seedDatabase() {
  ...
  for (const policy of seedPolicies) {
    try {
      await query(
        `INSERT INTO policies (title, category, source, url, source_date, content)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT DO NOTHING`,
        [policy.title, policy.category, policy.source, policy.url, policy.sourceDate, policy.content]
      );
