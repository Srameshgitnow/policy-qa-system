...
export async function ingestPolicy(
  ...
  try {
    ...
    const policyResult = await query(
      `INSERT INTO policies (title, category, source, url, content, source_date)
       VALUES ($1, $2, $3, $4, $5, CURRENT_DATE)
       RETURNING id`,
      [title, category, source, url, content]
    );
    ...
    for (let i = 0; i < chunks.length; i++) {
      await query(
        `INSERT INTO policy_chunks (policy_id, chunk_index, content, embedding, tokens)
         VALUES ($1, $2, $3, $4::vector, $5)`,
        [policyId, i, chunks[i], JSON.stringify(embeddings[i]), Math.ceil(chunks[i].length / 4)]
      );
    }
